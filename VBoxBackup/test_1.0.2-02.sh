...
#  copy RSA pub to target and set check-file
    ...
    echo -e "\n\tIf you still get prompted for a password, \n\tManually run 'ssh-copy-id root@$2'\n"
fi
...
#  Start Logging
echo -e "\n$RDATE" >> $LOGFILE
echo "Shutting down $1 for exporting..." >> $LOGFILE
#  Shutdown VM
...
if ! ping -c 1 $2 > /dev/null 2>&1
    then
        echo "Beginning export of $1" >> $TMPLOG
